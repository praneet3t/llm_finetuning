...
NUM_PROMPTS  = 100    # Use 100–200 for publication; set to 5 for a quick smoke test
...
def benchmark_latency(model_name, base_id, adapter_path, prompts):
    ...
    # 2) Warm‑up
    inp = tokenizer(prompts[0], return_tensors="pt", truncation=True, max_length=2048).to(model.device)
    with torch.no_grad():
    ...
    for prompt in tqdm(prompts, desc=f"  Generating ({model_name})"):
        ...
        t0 = time.time()
        with torch.no_grad():
            output= model.generate(**inp, max_new_tokens=MAX_TOKENS, do_sample=False)
            decoded = tokenizer.decode(output[0], skip_special_tokens=True)#rem
            num_tokens = len(tokenizer(decoded).input_ids)#rem
            print(f"Generated {num_tokens} tokens")#rem
        torch.cuda.synchronize()
        t1 = time.time()
